    ...
    <div class="max-w-2xl mx-auto px-3 pt-4">
            ...
            <div class="px-4 pb-4">
                <div class="flex items-end justify-between -mt-7 mb-2">
                    <div class="w-16 h-16 rounded-full bg-white dark:bg-neutral-900 border-4 border-white dark:border-neutral-900 overflow-hidden flex items-center justify-center">
                        <x-avatar :email="auth()->user()->email" class="w-14 h-14" />
                    </div>
                ...
                <h1 class="text-xl font-extrabold text-gray-900 dark:text-gray-100">{{ auth()->user()->name }}</h1>
                <p class="text-xs text-white/80 bg-yellow-400 rounded-full inline-block px-2.5 py-0.5 mt-2 text-gray-900">{{ '@' . auth()->user()->username }}</p>

                @if(auth()->user()->title)
                    <div class="mt-2">
                        <span class="inline-flex items-center gap-1 bg-yellow-100 dark:bg-yellow-950/50 text-yellow-700 dark:text-yellow-400 text-xs font-bold px-3 py-1 rounded-full">
        ...
        <div class="bg-white dark:bg-neutral-900 border border-gray-300 dark:border-neutral-800 rounded-lg mb-3 overflow-hidden">
            <div class="flex items-center justify-between px-3 py-2.5 border-b border-gray-100 dark:border-neutral-800">
                ...
                <span class="text-xs font-bold text-gray-500">{{ count($earnedIds) }}/{{ $badges->count() }}</span>
            </div>
            ...
            <div class="grid grid-cols-3 gap-2 p-3">
                @foreach($badges as $badge)
                    ...
                    <div class="rounded-xl p-3 text-center
                        {{ $earned ? 'border border-yellow-300 dark:border-yellow-800 bg-yellow-50/50 dark:bg-yellow-950/20' : 'border border-gray-100 dark:border-neutral-800 opacity-50' }}">
        ...
        {{-- ===== LINK MENU ===== --}}
        <a href="{{ route('letters.index') }}" class="flex items-center justify-between bg-white dark:bg-neutral-900 border border-gray-300 dark:border-neutral-800 rounded-lg p-3.5 mb-2 text-sm hover:border-yellow-400 dark:hover:border-yellow-600 transition">
            <span class="text-gray-700 dark:text-gray-200 font-bold">✉️ Surat untuk diri sendiri</span>
            <span class="text-gray-300 dark:text-gray-600">›</span>
        </a>

        <a href="{{ route('memories.index') }}" class="flex items-center justify-between bg-white dark:bg-neutral-900 border border-gray-300 dark:border-neutral-800 rounded-lg p-3.5 mb-2 text-sm hover:border-yellow-400 dark:hover:border-yellow-600 transition">
            <span class="text-gray-700 dark:text-gray-200 font-bold">📖 Perjalanan lu</span>
            <span class="text-gray-300 dark:text-gray-600">›</span>
        </a>

        <a href="{{ route('graves.index') }}" class="flex items-center justify-between bg-white dark:bg-neutral-900 border border-gray-300 dark:border-neutral-800 rounded-lg p-3.5 mb-2 text-sm hover:border-yellow-400 dark:hover:border-yellow-600 transition">
            <span class="text-gray-700 dark:text-gray-200 font-bold">🪦 Makam streak</span>
            <span class="text-gray-300 dark:text-gray-600">›</span>
        </a>

        <a href="{{ route('settings') }}" class="flex items-center justify-between bg-white dark:bg-neutral-900 border border-gray-300 dark:border-neutral-800 rounded-lg p-3.5 mb-4 text-sm hover:border-yellow-400 dark:hover:border-yellow-600 transition">
            <span class="text-gray-700 dark:text-gray-200 font-bold">⚙️ Pengaturan</span>
